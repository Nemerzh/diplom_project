...
import { createMeter, deleteMeter, getLines, getMeters, getSites, updateMeter } from "../../api";
...
export default function AdminMetersPage() {
  ...
  const load = async () => {
    const [m, s, ls] = await Promise.all([
      getMeters(),
      getSites(),
      getLines()
    ]);
    ...
    setSites(s);
    setLines(ls);
    if (!form.site_id && s.length) setForm((prev) => ({ ...prev, site_id: String(s[0].id), line_id: s[0].line_id ? String(s[0].line_id) : "" }));
  };
  ...
  const submit = async (e) => {
    ...
    try {
      const payload = {
        ...
        meter_type: form.meter_type.trim() || "electricity",
  ...
  const beginEdit = (meter) => {
    ...
    setForm({
      ...
      meter_role: meter.meter_role || "workshop_zone",
  ...
  return (
    <div style={styles.grid2}>
      ...
      <div style={styles.card}>
        ...
        <form onSubmit={addTemplate}>
          <p>
            ...
            <input style={styles.input} value={templateForm.label} onChange={(e) => setTemplateForm({ ...templateForm, label: e.target.value })} placeholder="Напр. Насоси" />
          </p>
          <p>
            <FieldLabel text="Роль лічильника для шаблону" />
            <select style={styles.input} value={templateForm.meter_role} onChange={(e) => setTemplateForm({ ...templateForm, meter_role: e.target.value })}>
              {ROLE_OPTIONS.map((r) => <option key={r.value} value={r.value}>{r.label}</option>)}
            </select>
          </p>
          <div style={styles.toolbar}>
            <button style={styles.button} type="submit">Додати шаблон</button>
          </div>
        </form>
        <DataTable
          ...
          rows={zoneTemplates.map((t) => [
            ...
            <button key={t.value} style={styles.buttonSecondary} onClick={() => removeTemplate(t.value)}>Видалити</button>
          ])}
        />
      </div>
      <div style={styles.card}>
        <h3>{editingId ? `Редагувати лічильник #${editingId}` : "Створити лічильник"}</h3>
        <form onSubmit={submit}>
          <p>
            <FieldLabel text="Об'єкт (Site)" />
            <select style={{ ...styles.input, ...(errors.site_id ? styles.inputError : {}) }} value={form.site_id} onChange={(e) => setForm({ ...form, site_id: e.target.value })}>
          ...
          <p>
            ...
            <select style={{ ...styles.input, ...(errors.line_id ? styles.inputError : {}) }} value={form.line_id} onChange={(e) => setForm({ ...form, line_id: e.target.value })}>
              <option value="">Оберіть лінію</option>
              {compatibleLines.map((ln) => <option key={ln.id} value={ln.id}>{ln.code} - {ln.name}</option>)}
            </select>
            <FieldError text={errors.line_id} />
          </p>
          <p>
            <FieldLabel text="Шаблон зони" />
            <select style={styles.input} value={form.zone_template} onChange={(e) => setForm({ ...form, zone_template: e.target.value })}>
              {zoneTemplates.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
            </select>
          </p>
          <p>
            ...
            <input
              ...
              readOnly
              placeholder="Цех 1 - головний ввід"
            />
            <FieldError text={errors.zone_name} />
          </p>
          <p>
            <FieldLabel text="Серійний номер" />
            <input style={{ ...styles.input, ...(errors.serial_number ? styles.inputError : {}) }} value={form.serial_number} onChange={(e) => setForm({ ...form, serial_number: e.target.value })} placeholder="Серійний номер" />
            <FieldError text={errors.serial_number} />
          </p>
          <p>
            <FieldLabel text="Тип лічильника" />
            <input style={styles.input} value={form.meter_type} onChange={(e) => setForm({ ...form, meter_type: e.target.value })} placeholder="Тип лічильника" />
          </p>
          <p>
            <FieldLabel text="Статус" />
            <input style={{ ...styles.input, ...(errors.status ? styles.inputError : {}) }} value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })} placeholder="Статус" />
      ...
      <div style={styles.card}>
        <h3>Лічильники</h3>
        <DataTable
          columns={["Об'єкт", "Лінія", "Зона", "Роль", "Серійний номер", "Тип", "Статус", "Дії"]}
          rows={meters.map((m) => [
            ...
            <div key={m.id} style={styles.actionGroup}>
              <button style={styles.buttonSecondary} onClick={() => beginEdit(m)}>Редагувати</button>
              <button style={styles.buttonSecondary} onClick={() => remove(m.id)}>Видалити</button>
            </div>
          ])}
        />
      </div>
